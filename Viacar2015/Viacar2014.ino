...
float r1, r2, x1, x2;
const float C1 = 254.f;
float C2 = 3300.f;
float rmax = 0.f;
float h, d, errorF, loadPercentF, loadMax;
...
CmdHandler* watch(const char* input)
{
    ...
    if (!strncmp(input, "w r1", 8))
    {
        return new WatchHandler(&r1);
    }
    else if (!strncmp(input, "w r2", 8))
    {
        return new WatchHandler(&r2);
    }
    ...
    else if (!strncmp(input, "w rmax", 8))
    {
        return new WatchHandler(&rmax);
    }
    else
    {
...
{
    r1 = float(analogRead(A0));
    r2 = float(analogRead(A1));
...
float count2dist(float r)
{   
    if (calSwitch.pressed() && r > rmax)
    {
		rmax = r;
		C2 = float(h*exp(rmax/C1));
	}
        
    float xsq = C2*h / exp(r/C1) - h*h;
